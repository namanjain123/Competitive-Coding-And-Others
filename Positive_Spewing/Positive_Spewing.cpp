...
void solve(){
  ...
  for(int i=0;i<n;i++){
    int added=0;
    int x=distl[i], y=distr[i];
    int toreachx=x-1, toreachy=y-1;
    if(a[i]){
      toreachx=min(1ll, toreachx);
      toreachy=min(1ll, toreachy);
    }
    toreachx=min(toreachx, y+1); 
    toreachy=min(toreachy, x+1);
    if(toreachx<k)
      added+=k-toreachx;
    if(toreachy<k)
      added+=k-toreachy;

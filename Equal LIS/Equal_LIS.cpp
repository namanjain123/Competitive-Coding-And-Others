...
void solve(){
  ...
  debug(val);
  //make last grp of size val
  //rest we can divide almost equally
  ...
  int strt=n-val;
  int ineach=(n-val)/(val-1); // n-val elements left
                             // will be divided in left val-1 groups
  int extra=(n-val)-ineach*(val-1);
  
  //running loop val-1 times
  val--;
  while(val--){
    int diff=ineach;
    if(extra){
    ...
    for(int i=strt-diff+1;i<=strt;i++)
      a.pb(i);
    strt-=diff;
  }
  // //yesno(1);

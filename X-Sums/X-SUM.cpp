...
int a[N][M], dl1[N][M], dl2[N][M], dr1[N][M], dr2[N][M];
...
void solve(){
  ...
  for (int i = 0; i <= n + 1;i++)
    for (int j = 0; j <= m + 1;j++)
      dl1[i][j] = dl2[i][j] = dr1[i][j] = dr2[i][j] = 0;
 
  for (int i = 1; i <= n;i++)
    for (int j = 1; j <= m;j++)
      dl1[i][j] = dl1[i - 1][j - 1] + a[i][j];
  
 
 
  for (int i = 1; i <= n;i++)
    for (int j = m; j >= 1;j--)
      dr1[i][j] = dr1[i - 1][j + 1] + a[i][j];
  ...
  for (int i = 1; i <= n;i++){
    for (int j = 1; j <= m;j++){
      int subans = 0;
      int d1 = 0, d2 = 0;
      int diff1 = min(n - i, m - j);
      d1 = dl1[i + diff1][j + diff1];
      int diff2 = min(j - 1, n - i);
      d2 = dr1[i + diff2][j - diff2];
      subans = d1 + d2;
      ans = max(ans, subans-a[i][j]);
    }
  }
    cout << ans << endl;
}
